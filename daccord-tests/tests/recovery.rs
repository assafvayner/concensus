...
fn teardown(
    handles: Vec<daccord::NodeHandle<String>>,
    decisions: Vec<daccord::DecisionReceiver<String>>,
    run_handles: Vec<tokio::task::JoinHandle<Result<(), daccord::NodeError>>>,
) {
    for h in handles {
        drop(h);
    }
...
async fn collect_until_value(
    rx: &mut daccord::DecisionReceiver<String>,
    target: &str,
    deadline: Duration,
) -> Vec<Decided<String>> {
...
async fn shared_raft_storage_handoff_preserves_term_and_log() {
    use daccord::{LogEntry, RaftStorage};
