#![cfg(feature = "redb")]
//! Recovery tests for the redb-backed [`PaxosStorage`] / [`RaftStorage`]
//! implementations.
//!
//! The trait-level tests exercise round-trip persistence across a `drop` +
//! reopen of the same file (or in-memory instance). The cluster-level tests
//! mirror `parity_recovery.rs` but route node 0's persistence through a
//! redb file on a `tempfile::TempDir`.
...
use daccord::{
    channel, LogEntry, Node, NodeId, PaxosConfig, PaxosMemoryStorage, PaxosStorage, PeerInfo,
    RaftConfig, RaftMemoryStorage, RaftStorage, RedbPaxosStorage, RedbRaftStorage,
};
...
async fn paxos_decisions_persist_across_reopen() {
    ...
    let path = fresh_path(&dir, "paxos.redb");

    {
        let mut s = RedbPaxosStorage::<String>::open(&path).unwrap();
        s.save_decision(0, "a".into()).await.unwrap();
        s.save_decision(1, "b".into()).await.unwrap();
    }

    let s = RedbPaxosStorage::<String>::open(&path).unwrap();
...
async fn paxos_in_memory_independent_per_instance() {
    let mut a = RedbPaxosStorage::<String>::open_in_memory().unwrap();
    let b = RedbPaxosStorage::<String>::open_in_memory().unwrap();
...
async fn raft_term_voted_for_persist_across_reopen() {
    let dir = TempDir::new().unwrap();
    let path = fresh_path(&dir, "raft.redb");

    let nid = NodeId::new("a", 42);
    {
        let mut s = RedbRaftStorage::<String>::open(&path).unwrap();
        s.save_term(7).await.unwrap();
        s.save_voted_for(Some(nid.clone())).await.unwrap();
    }

    {
        let s = RedbRaftStorage::<String>::open(&path).unwrap();
...
async fn raft_truncate_past_end_is_noop() {
    let mut s = RedbRaftStorage::<String>::open_in_memory().unwrap();
    s.append_log(&[LogEntry {
        term: 1,
        value: "a".into(),
    }])
...
async fn paxos_and_raft_share_same_db_file() {
    let dir = TempDir::new().unwrap();
    let path = fresh_path(&dir, "shared.redb");

    {
        let mut p = RedbPaxosStorage::<String>::open(&path).unwrap();
        p.save_decision(5, "paxos-only".into()).await.unwrap();
    }
    {
        let mut r = RedbRaftStorage::<String>::open(&path).unwrap();
    ...
    {
        let p = RedbPaxosStorage::<String>::open(&path).unwrap();
        let decs = p.load_decisions().await.unwrap();
        assert_eq!(decs, vec![(5, "paxos-only".to_string())]);
    }
    {
        let r = RedbRaftStorage::<String>::open(&path).unwrap();
...
fn build_paxos_cluster_with_redb(
    ids: &[NodeId; 3],
    storage0: RedbPaxosStorage<String>,
) -> (
...
async fn paxos_cluster_restart_recovers_decisions_via_redb() {
    let dir = TempDir::new().unwrap();
    let path = fresh_path(&dir, "paxos-cluster.redb");
    let ids: [NodeId; 3] = [
        NodeId::new("a", 1000),
        NodeId::new("b", 1000),
        NodeId::new("c", 1000),
    ...
    {
        let storage0 = RedbPaxosStorage::<String>::open(&path).unwrap();
        let (handles, mut decisions, run_handles) = build_paxos_cluster_with_redb(&ids, storage0);
        ...
        // redb holds an exclusive file lock per Database, so we must wait for
        // the node tasks to fully drop their storage before reopening below.
    ...
    {
        let storage = RedbPaxosStorage::<String>::open(&path).unwrap();
    ...
    let storage0 = RedbPaxosStorage::<String>::open(&path).unwrap();
    let (handles2, mut decisions2, run_handles2) = build_paxos_cluster_with_redb(&ids, storage0);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn raft_cluster_restart_persists_state_via_redb() {
    let dir = TempDir::new().unwrap();
    let path = fresh_path(&dir, "raft-cluster.redb");
    let ids: [NodeId; 3] = [
        NodeId::new("a", 1000),
        NodeId::new("b", 1000),
        NodeId::new("c", 1000),
    ...
    {
        let storage0 = RedbRaftStorage::<String>::open(&path).unwrap();
        let (tx0, rx0) = channel(64);
        let (tx1, rx1) = channel(64);
        let (tx2, rx2) = channel(64);
        let peers0 = vec![
        ...
        r1.abort();
        r2.abort();
        // redb holds an exclusive file lock per Database, so wait for the
        // node tasks to fully drop their storage before reopening below.
        let _ = r0.await;
        let _ = r1.await;
        let _ = r2.await;
    }
    ...
    // proposed count.
    let storage = RedbRaftStorage::<String>::open(&path).unwrap();
