...
All `docker compose` commands should be run from this directory (`daccord-demo/`):
...
The `daccord-cli` binary connects to a node's gRPC server. You can run it locally (requires Rust) or via `docker exec`.

### Running locally

```bash
# Build once
cargo build -p daccord-demo --bin daccord-cli

# Or run directly with cargo
cargo run -p daccord-demo --bin daccord-cli -- --addr localhost:50051 <command>
```

### Running via Docker

```bash
docker exec daccord-node-1-1 daccord-cli --addr localhost:50051 <command>
```
...
```bash
daccord-cli --addr localhost:50051 propose --value "my-value"
```
