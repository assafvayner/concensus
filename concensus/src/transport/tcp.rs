...
//!
//! Messages are framed as `[4-byte BE length][payload]`. Connections are
//! established lazily on the first send and automatically reconnected on
//! failure. The listener re-binds with exponential backoff if the accept
...
/// Sends length-prefixed messages to a single TCP peer.
///
/// The connection is established lazily on the first [`send`](MessageSender::send)
/// call and transparently reconnected if the previous connection was lost.
...
impl TcpSender {
    /// Creates a new sender targeting the given address. No connection is
...
/// Listens for incoming TCP connections and demultiplexes received messages
/// into a single stream.
///
/// Each accepted connection spawns a reader task that frames incoming bytes
/// using the same 4-byte length prefix. Messages from all connections are
/// funneled into a single internal channel consumed by
...
impl TcpReceiver {
    /// Binds a TCP listener on `addr` and starts accepting connections.
...
/// Factory for creating a matched set of TCP senders and a receiver.
pub struct TcpTransport;

impl TcpTransport {
    /// Creates [`PeerInfo`] entries for all peers and binds a [`TcpReceiver`]
    /// on `bind_addr`.
    ///
    /// This is the recommended way to set up TCP transport. It returns
